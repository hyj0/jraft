...
public:
    ...
    void setGroupCfg(const GroupCfg &groupCfg) override {
        string value = "keke";
        Status status;
        status = db->Put(leveldb::WriteOptions(), key_GroupCfg, value);
    ...
    void setRaftConfig(const jraft::Storage::RaftConfig &raftConfig) override {
        ...
        Status status = db->Put(leveldb::WriteOptions(), key_RaftConfig, json.dump());
        if (!status.ok()) {
    ...
    int setRaftLog(jraft::Storage::Log &log, int index) override {
        ...
        Status status = db->Put(leveldb::WriteOptions(), getRaftLogKey(index), json.dump());
        if (!status.ok()) {
            LOG_COUT << "put err:" << LOG_ENDL;
            return -1;
        }
